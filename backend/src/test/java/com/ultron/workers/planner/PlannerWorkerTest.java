...
/** Planner prioritisation via offline brain + RAG on in-memory H2. */
@DataJpaTest
class PlannerWorkerTest {
    ...
    void setUp() {
        ...
        MemoryService memoryService = new MemoryService(memoryRepository, gate, embedder);
        ...
        planner = new PlannerWorker(brain, rag);
    }

    @Test
    void producesAPlanFromTasks() {
        WorkerResult result = planner.handle(new WorkerRequest("plan",
            Map.of("goal", "ship phase 1", "tasks", List.of("write tests", "run build", "update docs"))));

        assertThat(result.success()).isTrue();
        assertThat(result.summary()).contains("[heuristic]");
        assertThat(result.detail()).contains("tasks=3");
    }

    @Test
    void requiresGoalOrTasks() {
        WorkerResult result = planner.handle(new WorkerRequest("plan", Map.of()));
